...
export default class Validator{
    ...
    constructor({selector, pattern ={}, method}){
        ...
        this.elementsForm = [...this.form.elements].filter(item => {
            // вернем элементы формы за исключением кнопок item.type.toLowerCase() !== 'button'
            return item.tagName.toLowerCase() !== 'button' && item.type.toLowerCase() !== 'button'; 
        });
        this.error = new Set();
    }
    ...
    init(){
        ...
        // обязательно нужно bind иначе теряем контекст вызова
        this.elementsForm.forEach(elem => elem.addEventListener('change', this.checkIt.bind(this))); 
        this.setPattern();
        this.form.addEventListener('submit', e => {
            // если size размер коллекции >0 то делаем event.preventDefault();
            if (this.error.size) { 
                e.preventDefault();
            } else {
    ...
    // если ошибка
    showError(elem){
        // добавим класс success если валидации произошла
        elem.classList.remove('success'); 
        // добавим класс error если произошла ошибка валидации
        elem.classList.add('error'); 
        // если у элемента справа есть класс 'validator-error' ничего не делаем
    ...
    showSuccess(elem){
        ...
        // проверим есть ли class ==='validator-error' у элемента справа 
        if (elem.nextElementSibling && elem.nextElementSibling.classList.contains('validator-error') ){ 
